...
BEGIN
    ...
    N.PROMEDIO, IF(N.PROMEDIO>=14, "APROBADO", "NO APROBADO") AS ESTADO
	FROM TPNOTA N
...
DELIMITER $$
CREATE PROCEDURE `USP_INFORMACION_PERSONAL`(P_ID_USUARIO INT)
BEGIN
SELECT 
	U.ID_USUARIO, 
	U.EMAIL, 
	...
	P.DNI, 
    P.FECHA_NACIMIENTO, 
    P.LUGAR_NAC_DIST, P.LUGAR_NAC_PROV, P.LUGAR_NAC_DEP, P.NACIONALIDAD,
	P.EDAD, P.GENERO, 
    P.UNIVERSITY, 
    P.PHONE, P.ADDRESS FROM TMPERSONA P
INNER JOIN TMUSUARIO U
ON P.ID_PERSONA = U.FK_ID_PERSONA
WHERE 
U.ID_USUARIO = P_ID_USUARIO;
...
DROP PROCEDURE IF EXISTS USP_ALUMNO_LIST;
DELIMITER //
CREATE PROCEDURE USP_ALUMNO_LIST(IN P_ID_USUARIO INT) 
BEGIN
	DECLARE V_COD_ESTUDIANTE VARCHAR(10);
    SET V_COD_ESTUDIANTE := (SELECT COD_ESTUDIANTE_CI FROM TMESTUDIANTE WHERE FK_ID_USUARIO = P_ID_USUARIO);
	
    SET lc_time_names = 'es_ES';
    SELECT CONCAT(DAYNAME(U.FECHA_ALTA), ', ', DAY(U.FECHA_ALTA), ' de ', MONTHNAME(U.FECHA_ALTA), ' del ', YEAR(U.FECHA_ALTA), ', ', DATE_FORMAT(time(U.FECHA_ALTA), '%H:%i')) AS FECHA_ALTA, 
    M.FK_COD_ESTUDIANTE_CI AS CODIGO, P.DNI, CONCAT(P.NOMBRE, ' ', P.APELLIDO_PAT, ' ', P.APELLIDO_MAT) AS NOMBRE, 
	CONCAT(I.NOM_IDIOMA, ' ',  L.NOM_NIVEL, ' ', C.CICLO) AS IDIOMA, N.PROMEDIO, PA.NOM_PERIODO, UPPER(MONTHNAME(PA.FECHA_INICIO)) AS MES, 
    CONCAT(DAYNAME(M.FECHA_MATRICULA), ', ', DAY(M.FECHA_MATRICULA), ' de ', MONTHNAME(M.FECHA_MATRICULA), ' del ', YEAR(M.FECHA_MATRICULA), ', ', DATE_FORMAT(time(M.FECHA_MATRICULA), '%H:%i')) AS FECHA_MATRICULA
	FROM TPNOTA N 
    LEFT JOIN TPMATRICULA M ON N.FK_ID_MATRICULA = M.ID_MATRICULA
	LEFT JOIN TPPROG_CURSO PC ON M.FK_ID_PROGCURSO = PC.ID_PROGCURSO
	LEFT JOIN TPPROG_DOC_CURSO PDC ON PC.FK_ID_PROG_DOC_CUR = PDC.ID_PROG_DOC_CUR
	...
	LEFT JOIN TMIDIOMA I ON C.FK_ID_IDIOMA = I.ID_IDIOMA
	LEFT JOIN TXNIVEL L ON C.FK_ID_NIVEL = L.ID_NIVEL
    LEFT JOIN TMESTUDIANTE E ON M.FK_COD_ESTUDIANTE_CI = E.COD_ESTUDIANTE_CI
    LEFT JOIN TMUSUARIO U ON E.FK_ID_USUARIO = U.ID_USUARIO
    LEFT JOIN TMPERSONA P ON U.FK_ID_PERSONA = P.ID_PERSONA
	WHERE U.ID_USUARIO = P_ID_USUARIO AND 
    M.ID_MATRICULA = (SELECT MAX(ID_MATRICULA) FROM TPMATRICULA WHERE FK_COD_ESTUDIANTE_CI=V_COD_ESTUDIANTE AND FK_ID_ESTADO_MATRICULA = 4);
END//
DELIMITER ;

DROP procedure IF EXISTS `USP_VOUCHERS_LIST`;
DELIMITER $$
CREATE PROCEDURE `USP_VOUCHERS_LIST` (P_ID_USUARIO INT)
BEGIN
SELECT V.COD, U.ID_USUARIO, U.EMAIL, P.DNI, V.SEC, V.DESCRIPCION, V.IMPORTE, V.FECHA, V.COMILLA AS ESTADO
FROM TMUSUARIO U 
INNER JOIN TMPERSONA P ON U.FK_ID_PERSONA = P.ID_PERSONA
INNER JOIN TMVOUCHER V ON V.NRODOCUMENTO = P.DNI
